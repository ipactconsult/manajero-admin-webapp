...
@Component({
  selector: 'ngx-publication-archive',
  templateUrl: './publication-archive.component.html',
  styleUrls: ['./publication-archive.component.scss']
})
export class PublicationArchiveComponent implements OnInit {
  ...
  getAllPublications() {
   ...
   onRestoreConfirm(p: Publication, id: string) {
    this.publicationService.cancelArchivePublication(p,id).subscribe(
      () => {
        this.showToast('success', 'Restored Successfully',
          'Publicatin  restored !!');
        this.router.navigate(['/CommunicationMarketing/Publication']).then(() => {
