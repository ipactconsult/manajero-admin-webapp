...
@Component({
  selector: 'ngx-table-releases',
  templateUrl: './table-releases.component.html',
  styleUrls: ['./table-releases.component.scss']
})
export class TableReleasesComponent implements OnInit {
  ...
  getReleases()
  {
    this.releaseService.findAll().subscribe(
      (data : Release[]) =>{this.releases = data.filter(
        (r=>r.releaseStatus=== 'Pending'|| r.releaseStatus=== 'Validate'|| r.releaseStatus=== 'Rejected')
      );},
      (err)=>{console.log(err);}
    )
  }
  ...
  getReleasesAsc()
  {
    this.releaseService.findAllASC().subscribe(
      (data : Release[]) =>{this.releases = data.filter(
        (d =>
          //@ts-ignore
          d.releaseStatus==='Pending'||d.releaseStatus==='Validate'||d.releaseStatus==='Rejected'
      )
      );},
      (err)=>{console.log(err);}
    )
